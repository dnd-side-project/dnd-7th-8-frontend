...
const itemGutterRatio = ({ perPage, gutter }) => ((perPage - 1) * gutter) / perPage;
...
const transformValue = ({ position, perPage, gutter }) => {
    ...
    const positionTimesItemGutterRatio = position * (-itemGutterRatio({ perPage, gutter }) + gutter);
...
const Item = styled.div`
    display: flex;
    flex-basis: ${(props) => `calc(${itemWidth(props.perPage)}% - ${itemGutterRatio(props)}px)`};
...
const useCarousel = ({ p = 1, length, perPage = 3 }) => {
    ...
    return (
        <Container>
            <Slide>
                ...
                <ItemsContainer perPage={perPage} position={position} gutter={gutter} {...handlers}>
...
const SliderSection = ({ recommandItems }: any) => (
    <Carousel perPage={1} gutter={1}>
